...
* id V(p1, v?) = (g_(v,p1)-M*gi_(v)) * pow(spA(f1,n1),-1) * ketA(n1,v);
*id U(p3, v?) = ketA(p3,v);
*id VBar(p4,v?) = braA(p4,v);
*id UBar(p2,v?) = braA(n2,v) * (g_(v,p2)+M*gi_(v)) * pow(spA(f2,n2),-1);

*id g_(v?, ?v0l0, v0l1?, ?v0l2) * EpsStar(v0l1?, p5) = pow(2, -1/2)*pow(spA(k1,p5),-1) * g_(v, ?v0l0) * 2 * (ketA(k1,v)*braB(p5,v)+ketB(p5,v)*braA(k1,v)) * g_(v, ?v0l2);
*.sort
